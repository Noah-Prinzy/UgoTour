...
function render(){
  list.innerHTML=""; empty.hidden=places.length!==0; list.hidden=places.length===0; status.textContent=`${places.length} saved place${places.length===1?"":"s"}`;
  places.forEach((place)=>{
    const card=document.createElement("article"); card.className="saved-card";
    card.innerHTML=`<a class="saved-card-link" href="${detailMapUrl(place)}"><img src="${esc(resolveAssetPath(place.imageUrl,".."))}" alt="${esc(place.name)}" loading="lazy"/><div><span class="tag">${esc(place.category||place.placeType)}</span><h2>${esc(place.name)}</h2><p>${esc(place.destinationName?`Near ${place.destinationName}`:(place.region||"Uganda"))}</p><span>Find on map →</span></div></a><button class="saved-remove" type="button" aria-label="Remove ${esc(place.name)} from saved places">♥</button>`;
